...
public class ModuleGreetingAcceptComamndImpl implements AcceptCommand {
    ...
    public ModuleGreetingAcceptComamndImpl(){
        InputStream in = this.getClass().getClassLoader().getResourceAsStream("module_greeting_commands.txt");
        String str=ProjectDoraemonUltil.getStringFromInputStream(in);
        String[] arr=str.split("\n");
        possibleCommands=Arrays.asList(arr);
        InputStream in1 = this.getClass().getClassLoader().getResourceAsStream("module_greeting_contexts.txt");
